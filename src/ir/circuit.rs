...
// This macro invocation defines the public-facing circuit API, which has several pieces:
//
// * `trait CircuitTrait<'a>`: the low-level circuit API, with methods like `gate(Gatekind)`.  This
//   is implemented by both the base `Circuit` and by adapters.  Methods marked `overridable` below
//   become part of this trait.
//
// * `trait CircuitExt<'a>`: the high-level circuit API, with methods like `add(Wire, Wire)`.  The
//   macro generates a blanket impl `CircuitExt for T where T: CircuitTrait`, which applies
//   automatically to the base `Circuit` and adapters.  Adapters cannot override the methods in
//   this trait (since `impl CircuitExt for Adapter<C>` would conflict with the blanket impl).
//   Non-`overridable` methods below become part of this trait.
//
// * `struct DynCircuit<'a>`: similar to `dyn CircuitTrait`, but provides the entire `CircuitTrait`
//   API, including generic (non-object-safe) methods.
//
// * `impl<'a> CircuitTrait<'a> for CircuitBase<'a>`
//
// * `impl<'a> CircuitTrait<'a> for DynCircuit<'a>`
//
// Clients of the circuit API should generally accept `c: &impl CircuitTrait<'a>` and also import
// `CircuitExt`, which allows them to call any of the methods defined in this macro.  When type
// erasure is needed, use `&DynCircuit<'a>` instead.  `DynCircuit` implements `CircuitTrait`, and
// `&impl CircuitTrait` can be converted to `DynCircuit` via `DynCircuit::new(c)`, so the two forms
// are compatible.
//
// Circuit adapters should implmement `CircuitTrait` as follows:
//
//      impl<'a, C: CircuitTrait<'a>> CircuitTrait<'a> for MyAdapter<C> {
//          type Inner = C;
//          fn inner(&self) -> &C { &self.inner }
//
//          fn gate(&self, gk: GateKind<'a>) -> Wire<'a> { ... }
//          // ...and/or other override methods
//      }
//
// The adapter can redefine any `overridable` method in this `impl`.  Most adapters will override
// `gate`, so they can rewrite any gates constructed through the adapter (including via helper
// methods such as `add`).  The default for any methods not overridden is to dispatch to the same
// method on `self.inner()`.
//
// See `overridable-trait/src/lib.rs` for details on the definitions generated by this macro.
