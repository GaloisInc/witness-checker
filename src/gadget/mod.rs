...
pub fn test_gadget_eval<'a, I>(c: &'a impl CircuitTrait<'a>, wires: I) -> usize
where I: IntoIterator<Item = Wire<'a>> {
    ...
    for wire in circuit::walk_wires(wires) {
        ...
        let xs = ws.iter().map(|&w| ev.eval_wire(w)).collect::<Vec<_>>();

        let eval_result = gk.eval(&tys, &xs);
        let decomp_result = ev.eval_wire(gk.decompose(c.as_ref(), ws));
