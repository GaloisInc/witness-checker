...
use crate::ir::circuit::{CircuitTrait, Ty, Wire, GateKind, TyKind, IntSize, BinOp, ShiftOp, CmpOp};
...
fn extend_integer_ty<'a>(c: &impl CircuitTrait<'a>, ty: Ty) -> Ty<'a> {
    match *ty {
...
fn normalize_64<'a>(c: &impl CircuitTrait<'a>, w: Wire<'a>, ty: Ty) -> Wire<'a> {
...
fn make_shifted_lit<'a>(c: &impl CircuitTrait<'a>, x: BigUint, ty: Ty<'a>) -> Option<Wire<'a>> {
...
fn make_split_lit<'a>(c: &impl CircuitTrait<'a>, x: BigUint, ty: Ty<'a>) -> Wire<'a> {
